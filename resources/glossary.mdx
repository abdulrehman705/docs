---
title: "Buildmarkets API Glossary: Key Terms and Definitions"
sidebarTitle: "Glossary"
description: "Definitions for every term used across the Buildmarkets API — from accounts and wallets to IPS, KYC, and order time-in-force values."
---

This glossary defines the key terms used throughout Buildmarkets' API, guides, and reference documentation. When you encounter an unfamiliar term in a response payload or endpoint description, look it up here.
...
| **ACH** | Automated Clearing House — the electronic bank transfer network used for deposits and withdrawals. |
| **Asset** | A tradable financial instrument (stock, ETF) available through Buildmarkets. |
...
| **Custodian** | A regulated financial institution that holds securities on behalf of investors. Buildmarkets routes to custodians automatically. |
...
| **Hosted Wallet** | Default wallet type where Buildmarkets manages the cash balance directly. |
...
| **Investor Profile** | A comprehensive financial picture: demographics, employment, tax, insurance, estate, cash flow, and preferences. |
...
| **KYC** | Know Your Customer — the identity verification process required before trading. |
...
| **Time in Force** | How long an order remains active: `day` (current session), `gtc` (until filled or canceled), `ioc` (immediate or cancel), `fok` (fill or kill). |
| **Trailing Stop** | A stop order where the stop price follows the market price by a fixed offset. |
| **Wallet** | The cash balance container for an investment account. Receives deposits, funds trades, and disburses withdrawals. |
