...
description: "Reference for every TAPP Engine webhook event type plus signature verification, delivery guarantees, and reliable handler best practices."
---

This page documents every event type that TAPP Engine can deliver to your webhook endpoint, organized by domain. Each entry includes the event name, when it fires, and the full JSON payload structure.
...
Fires when an order has been accepted by TAPP and submitted to the market. The order is now in `pending_new` or `new` status.
...
Fires when an order is rejected — either by TAPP validation or by the downstream execution venue. The `rejection_reason` explains why.
...
app.post('/webhooks/tapp', (req, res) => {
...
Every webhook payload TAPP sends is cryptographically signed. Verifying this signature before processing an event is **critical** — it ensures the payload genuinely came from TAPP and has not been tampered with. This page explains how to verify signatures, what delivery guarantees TAPP provides, how to inspect delivery history, and best practices for building a reliable webhook handler.

* * *

### Signature verification

When you register a webhook, TAPP returns a `secret` that begins with `whsec_`. TAPP uses this secret to compute an HMAC-SHA256 signature of the raw request body and attaches it to every delivery via the `X-TAPP-Signature` header.
...
2.  Read the `X-TAPP-Signature` header
...
app.use('/webhooks/tapp', express.raw({ type: 'application/json' }));

const WEBHOOK_SECRET = process.env.TAPP_WEBHOOK_SECRET; // your whsec_... value

app.post('/webhooks/tapp', (req, res) => {
  const signature = req.headers['x-tapp-signature'];
  if (!signature) {
...
app = Flask(__name__)
WEBHOOK_SECRET = os.environ['TAPP_WEBHOOK_SECRET'].encode()

@app.route('/webhooks/tapp', methods=['POST'])
def handle_webhook():
    signature = request.headers.get('X-TAPP-Signature')
    if not signature:
...
TAPP treats any non-`2xx` response (including `3xx` redirects) as a delivery failure and will retry.

#### Retry schedule

If your endpoint does not return a `2xx` within 10 seconds, TAPP retries delivery with exponential backoff:
...
After 5 failed retries, the delivery is marked as permanently failed. TAPP does not attempt further delivery for that event. Check the delivery history endpoint to identify and investigate failed events.
...
TAPP guarantees **at-least-once delivery** — every event will be delivered at least once, but rare network conditions can result in duplicate deliveries. Use the `id` field from the payload envelope as an idempotency key to detect and safely ignore duplicates.
...
curl https://api.tappengine.com/v1/webhooks/wh_a1b2c3d4-e5f6-7a8b-9c0d-e1f2a3b4c5d6/deliveries \
  -H "Authorization: Bearer $TAPP_API_KEY"
```
...
If your endpoint is temporarily unavailable (e.g., during a deployment), you can pause delivery by setting the webhook `status` to `disabled` via `PATCH /v1/webhooks/{webhookId}`. TAPP will not attempt delivery while the webhook is disabled.

> **Note:** TAPP does not queue events that occur while a webhook is disabled. Events that fire during a disabled window will not be retroactively delivered. Use the relevant TAPP API endpoints to reconcile missed state changes if needed.
