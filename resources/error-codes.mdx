...
description: "Reference table of TAPP Engine HTTP status codes and error codes for authentication, validation, account, trading, and funding errors plus rate limits."
---

TAPP Engine uses standard HTTP status codes along with structured JSON error bodies to communicate what went wrong. This page is your reference for understanding and handling error responses.
...
| `request_id` | A unique ID for this request — include this when contacting TAPP Engine support |
...
| Code | Meaning |
|------|---------|
| `500 Internal Server Error` | An unexpected error occurred on TAPP Engine's side. |
...
TAPP Engine enforces rate limits to ensure platform stability for all partners.

| Limit | Default |
|-------|---------|
| Requests per second (per API key) | 10 |
| Requests per minute (per API key) | 300 |
| Requests per day (per partner) | Contact your TAPP representative |
...
Always log the `request_id` from error responses. When contacting TAPP Engine support, provide this ID so the support team can trace the request through internal systems.
