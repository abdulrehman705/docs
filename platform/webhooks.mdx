...
description: "Register and manage HTTPS webhook endpoints to receive real-time event notifications from TAPP Engine instead of polling the API."
---

Webhooks let your application receive real-time notifications when events occur within TAPP Engine — account approvals, funding transfers, order fills, and more. Rather than polling the API repeatedly to check for status changes, you register an HTTPS endpoint and TAPP pushes event payloads to it as things happen.
...
```
      Event occurs in TAPP         TAPP sends HTTP POST          Your server
...
2.  **Receive** a signing secret — used to verify that payloads come from TAPP
3.  **TAPP delivers** a JSON payload to your URL via HTTP POST whenever a subscribed event fires
4.  **Your server** validates the signature, processes the event, and returns a `2xx` status
5.  **TAPP retries** failed deliveries automatically (see [Security & Delivery](/guides/operations/webhook-setup))
...
|-------|------|----------|-------------|
| `url` | string (URI) | ✅ | HTTPS URL where TAPP will POST event payloads. Must be a valid `https://` URL |
...
curl -X POST https://api.tappengine.com/v1/webhooks \
  ...
  -d '{
    "url": "https://myapp.com/webhooks/tapp",
    "events": []
  }'
```
...
curl -X PATCH https://api.tappengine.com/v1/webhooks/wh_a1b2c3d4-e5f6-7a8b-9c0d-e1f2a3b4c5d6 \
  ...
  -H "Content-Type: application/json" \
  -d '{
    "url": "https://myapp.com/webhooks/tapp-v2",
...
Triggers a synthetic test payload to be sent to the webhook's registered URL. Use this to verify that your endpoint is reachable and correctly processing TAPP payloads without waiting for a real event to occur.
...
TAPP sends a test payload with `"event_type": "webhook.test"` to the registered URL. Your endpoint should respond with a `2xx` status. Check the [delivery history](/guides/operations/webhook-setup) to see whether it was received successfully.
