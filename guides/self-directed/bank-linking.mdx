...
description: "Create, list, and remove ACH relationships that link an external bank account to a TAPP brokerage account for deposits and withdrawals."
---

Before a user can move money into or out of their brokerage account, you must establish a trusted link between their TAPP account and an external bank account they own. That link is called an **ACH relationship**. It is the permanent, reusable funding rail that makes all subsequent deposits and withdrawals possible.

An ACH relationship represents a verified connection to a specific external bank account — identified by routing number and account number. Once created and approved, the relationship can be used repeatedly for funding activity without re-entering bank credentials. A single TAPP account can hold multiple ACH relationships (e.g., a checking account and a savings account), but only **APPROVED** relationships can be used to initiate transfers.

## ACH relationship lifecycle

When a relationship is created, it passes through a short verification workflow before becoming active. TAPP uses third-party instant verification in production — there are no micro-deposits.
...
Links an external bank account to the specified TAPP brokerage account. TAPP automatically attempts instant verification via a third-party bank data provider. If verification succeeds, the relationship moves to `APPROVED` without any manual review.
...
|-----------|------|----------|-------------|
| `accountId` | string (UUID) | ✅ | The TAPP account ID to link the bank account to |
...
| `account_id` | string (UUID) | The TAPP account this relationship is linked to |
...
Returns all ACH relationships associated with a TAPP account, across all statuses. Use the `status` field in each object to determine which relationships are available for transfers.
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `accountId` | string (UUID) | ✅ | The TAPP account ID |
...
| `422` | `ACCOUNT_NOT_ACTIVE` | The TAPP account is not in `ACTIVE` status |

## Webhook events

TAPP fires webhook events when an ACH relationship changes status. Subscribe to these to drive real-time UI updates.
