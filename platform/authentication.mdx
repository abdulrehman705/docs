...
description: "How TAPP Engine API key authentication works, including generating, listing, and revoking scoped key pairs and security best practices."
---

The TAPP Engine API uses **API key authentication**. Every request must include a valid key pair. This page explains how authentication works, how to generate and manage keys, and security best practices.

## How authentication works

Every TAPP Engine API request requires two HTTP headers:
...
curl https://dev-tapp-api.tappengine.com/v1/accounts \
  -H "X-API-Key: tapp_key_abc123" \
  -H "X-API-Secret: tapp_secret_xyz789"
```
...
Log into the TAPP Engine Partner Dashboard, navigate to **Settings → API Keys**, and click **+ Generate Key**. Give the key a descriptive label (e.g., `"production-backend"` or `"sandbox-frontend"`) and select the required scopes.
...
```json
{
  "id": "key-001",
  "label": "backend-service-prod",
  "api_key": "tapp_key_abc123",
  "api_secret": "tapp_secret_xyz789",
  "scopes": ["accounts:read", "trading:write", "marketdata:read"],
