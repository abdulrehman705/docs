...
Once a user has an [approved ACH relationship](/guides/self-directed/bank-linking), they can move money between their external bank account and their TAPP brokerage account. TAPP exposes two endpoints for this:
...
> **Settlement timing:** "Standard ACH deposits typically settle in 1–3 business days. TAPP may apply **instant buying power** for deposits up to a configured limit, allowing users to trade before funds fully settle."
...
Pulls funds from the user's external bank account into their TAPP brokerage account. The specified ACH relationship must be in `APPROVED` status.
...
|-----------|------|----------|-------------|
| `accountId` | string (UUID) | ✅ | The TAPP account to deposit funds into |
...
| `account_id` | string (UUID) | The TAPP account receiving the funds |
...
Pushes settled cash from the user's TAPP brokerage account back to their external bank account. Only **settled** cash can be withdrawn — unsettled funds from recent trades or pending deposits are not eligible.
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `accountId` | string (UUID) | ✅ | The TAPP account to withdraw funds from |
...
|-------|------|-------------|
| `id` | string (UUID) | Unique identifier for this funding activity record |
| `account_id` | string (UUID) | The TAPP account funds are being withdrawn from |
