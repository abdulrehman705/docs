---
title: "Bluum Finance: Investment Infrastructure API for Fintechs"
sidebarTitle: "Introduction"
description: "Bluum Finance is an investment infrastructure API. Embed brokerage accounts, order execution, ACH funding, KYC, and compliance in one integration."
---

Bluum Finance is an investment infrastructure API for fintech companies. It sits between your application and the financial system — handling account onboarding, identity verification, custodian routing, order execution, wallet management, and regulatory compliance. You build the user experience; Bluum provides the rails.

## How it works

Your application makes REST calls to the Bluum API. Bluum orchestrates all downstream operations — creating brokerage accounts with custodians, routing orders to exchanges, initiating ACH transfers, running identity verification — and returns clean, normalized responses. Webhook events notify you in real time when asynchronous operations complete (order fills, deposit arrivals, KYC decisions).
